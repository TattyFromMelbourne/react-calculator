...
function CalculatorDisplay(props) {
	...
	if (formattedValue === 'NaN') {
		...
		if (formattedValue.length > maxPrecision - 1) {
			scientificNotation = parseFloat(value).toExponential(
				maxPrecision - 4
			); // Allow at least 4 characters (for scientific notation e.g. e+14) in the output string
			if (
				...
				scientificNotation = parseFloat(
					value
				).toExponential(maxPrecision - 1);
	...
	scaleDown =
		`${formattedValue}`.length > maxCharsAtFullSize
			? scaleFactor
			: 'scale(1)';
	return (
...
class Calculator extends Component {
	...
	processPoint(newKeyValue) {
		const { displayValue, waitingForOperand } = this.state;
		const needPoint =
			`${displayValue}`.indexOf('.') === -1 ? true : false;
		let newDisplayValue = null;
	...
	processPercentage(newKeyValue) {
		...
		const newDisplayValue =
			parseFloat(displayValue).toPrecision(maxPrecision) /
			100;
		this.setState({
			displayValue: `${newDisplayValue}`,
			waitingForOperand: false,
